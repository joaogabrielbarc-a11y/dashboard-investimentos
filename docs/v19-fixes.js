...
performanceV19=function(h){
  const current=Math.max(0,+h?.value||0),invested=investedValueV19(h),estimated=!finiteFieldV19(h?.avgPriceBRL)&&h?.avgCurrency==='USD',avgCurrency=h?.avgCurrency||'BRL',currentCurrency=h?.currentCurrency||(finiteFieldV19(h?.currentPriceNative)?avgCurrency:'BRL'),sameCurrency=avgCurrency===currentCurrency;
  const nativePct=sameCurrency&&finiteFieldV19(h?.avgPriceNative)&&+h.avgPriceNative>0&&finiteFieldV19(h?.currentPriceNative)?((+h.currentPriceNative/+h.avgPriceNative)-1)*100:null,delta=Number.isFinite(invested)?current-invested:null,pctValue=Number.isFinite(invested)&&invested>0?delta/invested*100:null;return {current,invested,delta,pct:Number.isFinite(nativePct)?nativePct:pctValue,estimated};
};
