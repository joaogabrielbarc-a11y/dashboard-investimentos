...
function ensureCss(){let link=document.querySelector('link[href^="v32-allocation-controls.css"]');if(!link){link=document.createElement('link');link.rel='stylesheet';document.head.appendChild(link);}link.href='v32-allocation-controls.css?v=32.1';}
...
function updateAllocationState(input){
  ...
  if(input.dataset.targetV25){const asset=state.assets.find(x=>x.id===input.dataset.targetV25);if(!asset)return false;asset.target=value;if(state.autoAportes)state.aportes={};save();render();return true;}
  if(input.id==='macroBandInputV26'||input.id==='globalBandV25'){state.band=value;const legacy=document.getElementById('band');if(legacy)legacy.value=value;save();render();return true;}
  if(input.dataset.bandClassV18){v18Plan.bands[input.dataset.bandClassV18]=value;saveV18Plan();render();return true;}
  if(input.dataset.segmentTargetV18){const plan=ensureSegmentPlanV18(input.dataset.segmentClassV18),segment=plan.find(x=>x.id===input.dataset.segmentTargetV18);if(!segment)return false;segment.target=value;saveV18Plan();render();return true;}
  return false;
}
