...
function render(){
  ...
  $('#kpis').innerHTML=`<article class="card kpi"><span>Patrimônio consolidado</span><strong>${fmt.format(total)}</strong><small>Carteira de longo prazo</small></article><article class="card kpi"><span>Aporte do mês</span><strong>${fmt.format(budget)}</strong><small>Orçamento disponível</small></article><article class="card kpi"><span>Aportes planejados</span><strong>${fmt.format(planned)}</strong><small>Soma dos valores editados</small></article><article class="card kpi"><span>Classes balanceadas</span><strong>${balanced}/${rows.length}</strong><small>Banda atual de ±${state.band}%</small></article><article class="card kpi accent"><span>Maior prioridade</span><strong>${priority?.name||'—'}</strong><small>${priority?`${pct(priority.currentWeight)} atual vs ${pct(priority.target)} alvo`:'—'}</small></article>`;
  ...
  $('#rows').innerHTML=rows.map(a=>`<tr><td><div class="assetName"><span class="dot" style="background:${colors[a.index%colors.length]}"></span>${a.name}</div></td><td><div class="cellInput"><span>R$</span><input data-id="${a.id}" data-field="current" type="number" min="0" step="10" value="${a.current}"></div></td><td><strong>${pct(a.currentWeight)}</strong><div class="bar"><i style="width:${Math.min(100,a.currentWeight/Math.max(1,a.target)*70)}%"></i></div></td><td><div class="targetInput"><input data-id="${a.id}" data-field="target" type="number" min="0" max="100" step="1" value="${a.target}"><span>%</span></div></td><td>${pct(a.min)} – ${pct(a.max)}</td><td><span class="pill ${a.balanced?'ok':'warn'}">${a.balanced?'OK':'NOK'}</span></td><td><div class="cellInput aporteInput"><span>R$</span><input data-aporte="${a.id}" type="number" min="0" step="10" value="${a.aporte.toFixed(2)}"></div></td><td><div class="postWeight"><strong class="${a.postBalanced?'positive':''}">${pct(a.postWeight)}</strong><small>${a.postBalanced?'dentro':'fora'} da banda após aporte</small></div></td></tr>`).join('');
...
$('#reset').onclick=()=>{state={assets:structuredClone(initialAssets),contribution:1500,band:25,aportes:{},autoAportes:true,showIdeal:false};save();render();};
render();
